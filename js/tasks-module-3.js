...
const SIZES = {
  ...
  MEDIUM: { name: 'medium', price: 15, cal: 50, time: 7 },
}
const STUFFING = {
...
const pizza = {
  ...
  changeSize(newSize) {
    this.size = newSize
    return `Size changing to ${newSize.name}`
  },
  ...
  deleteStuffing(stuffing) {
    if (this.stuffing.length === 0) return `Error! Stuffing is empty!`
  ...
  deleteSouce(souce) {
    if (this.souces.length === 0) return `Error! Souces is empty!`
  ...
  getInfo() {
    if (this.stuffing.length === 0 && this.souces.length === 0) {
      return `Error! Stuffing and souces are empty!`
    }

    if (this.stuffing.length === 0) return `Error! Stuffing is empty!`
    if (this.souces.length === 0) return `Error! Souces is empty!`

    const sizeInfo = this.size.name
    const stuffingInfo = this.stuffing.map(({ name }) => name).join(',')
    const soucesInfo = this.souces.map(({ name }) => name).join(',')

    return `Pizza size: ${sizeInfo}. Stuffing: ${stuffingInfo}. Souces: ${soucesInfo}. ${this.geCalories()} ${this.getPrice()}`
  },

  isValid() {
    return this.size && this.stuffing.length > 0 && this.souces.length > 0
  },
}
...
// console.log(pizza.deleteStuffing(STUFFING.BEACON))
